...
function createDensityMap(optionsJsonPath) {
  ...
  combineJson(optionsJsonPath, function (od) {
    optionData = od;
    //排序
...
function combineJson(optionsJsonPath, callback) {
  fetchDensityData(function (densityData) {/////////////////////////
    //////////////////////////
    $.get(optionsJsonPath, function (optionJson) {
      optionJson.series[0].data = densityData;
      callback(optionJson);
      return;
    });
  });
...
function getPieces(data) {
  ...
  var min = data.series[0].data[data.series[0].data.length - 1].value;
  var max = data.series[0].data[0].value;
  var splitNumber = 10;
  var pieces = [];
